...
HDPrivateKey::HDPrivateKey(const uint8_t secret[32], const uint8_t chain_code[32],
			   uint8_t key_depth, const uint8_t parent_fingerprint_arr[4],
			   uint32_t child_number, const Network *net, ScriptType key_type)
{
	init();
...
HDPrivateKey::HDPrivateKey(const char *mnemonic, size_t mnemonicSize, const char *password,
...
HDPrivateKey::HDPrivateKey(String mnemonic, String password, const Network *net,
			   void (*progress_callback)(float))
{
	init();
...
#if USE_ARDUINO_STRING || USE_STD_STRING
int HDPrivateKey::fromMnemonic(String mnemonic, String password, const Network *net,
			       void (*progress_callback)(float))
{
	return fromMnemonic(mnemonic.c_str(), mnemonic.length(), password.c_str(),
			    password.length(), net, progress_callback);
}
#endif
...
HDPublicKey HDPrivateKey::xpub() const
{
	...
	return HDPublicKey(p.point, chainCode, depth, parentFingerprint, childNumber, network,
			   type);
}
...
HDPublicKey::HDPublicKey(const uint8_t p[64], const uint8_t chain_code[32], uint8_t key_depth,
			 const uint8_t parent_fingerprint_arr[4], uint32_t child_number,
			 const Network *net, ScriptType key_type)
{
	reset();
	memcpy(point, p, 64);
	memzero(prefix, 4);
