...
size_t mnemonicToEntropy(const char *mnemonic, size_t mnemonicLen, uint8_t *output,
			 size_t outputLen)
{
	int num_words = 1;
...
{
	return generateMnemonic(numWords, (uint8_t *)entropy_string.c_str(),
				strlen(entropy_string.c_str()));
}
...
size_t Signature::from_stream(ParseStream *stream)
{
	...
	if (stream->available() && tot[2] == 33 &&
	    bytes_parsed + bytes_read < (size_t)4 + tot[1] + 3) {
		c = stream->read();
		bytes_read++;
		if (c != 0) {
			status = PARSING_FAILED;
