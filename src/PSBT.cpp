...
size_t descriptorChecksum(const char *span, size_t spanLen, char *output, size_t outputSize)
{
	 ...
	 *  - The most common 'unprotected' descriptor characters (hex, keypaths) are in the first
	 * group of 32.
	 *  - Case errors cause an offset that's a multiple of 32.
	 *  - As many alphabetic characters are in the same group (while following the above
	 * restrictions).
	 *
	 * If p(x) gives the position of a character c in this character set, every group of 3
	 * characters (a,b,c) is encoded as the 4 symbols (p(a) & 31, p(b) & 31, p(c) & 31, (p(a) /
	 * 32) + 3 * (p(b) / 32) + 9 * (p(c) / 32). This means that changes that only affect the
	 * lower 5 bits of the position, or only the higher 2 bits, will just affect a single
	 * symbol.
	 *
	 * As a result, within-group-of-32 errors count as 1 symbol, as do cross-group errors that
	 * don't affect the position within the groups.
	 */
	...
	static const char *INPUT_CHARSET =
		"0123456789()[],'/"
		"*abcdefgh@:$%{}IJKLMNOPQRSTUVWXYZ&+-.;<=>?!^_|~ijklmnopqrstuvwxyzABCDEFGH`#\"\\ ";
	...
	for (size_t i = 0; i < spanLen; i++) {
		...
		c = PolyMod(c, pos & 31);   // Emit a symbol for the position inside the group, for
					    // every character.
	...
	memset(output, ' ', 8);
	for (int j = 0; j < 8; ++j) {
		output[j] = CHECKSUM_CHARSET[(c >> (5 * (7 - j))) & 31];
	}
	return 8;
}
...
{
	char checksum[10] = {0};
	descriptorChecksum(descriptor.c_str(), strlen(descriptor.c_str()), checksum,
			   sizeof(checksum));
...
size_t PSBT::from_stream(ParseStream *s)
{
			...
			for (size_t i = 0; i < tx.outputsNumber; i++) {
				if (txOutsMeta[i].derivationsLen > 0) {
					for (size_t j = 0; j < txOutsMeta[i].derivationsLen; j++) {
						if (txOutsMeta[i].derivations[j].derivationLen >
						    0) {
							free(txOutsMeta[i]
								     .derivations[j]
								     .derivation);
	...
	if (last_key_pos == 5 && value.getStatus() == PARSING_DONE &&
	    key.getStatus() == PARSING_DONE) {
	...
	while (s->available() && current_section < sections_number) {
		...
		if (key.getStatus() == PARSING_FAILED || value.getStatus() == PARSING_FAILED) {
			status = PARSING_FAILED;
			ubtc_errno = (key.getStatus() == PARSING_FAILED) ? UBTC_ERR_PSBT_KEY
									 : UBTC_ERR_PSBT_VALUE;
			bytes_parsed += bytes_read;
			return bytes_read;
		}
...
int PSBT::add(uint8_t section, const Script *k, const Script *v)
{
	...
	if (section < 1 + tx.inputsNumber) { // input section
		...
		case 0: { // PSBT_IN_NON_WITNESS_UTXO
			...
			Tx tempTx;
			tempTx.parse(val_arr + lenVarInt(v->length()),
				     v->length() - lenVarInt(v->length()));
		...
		case 1: { // PSBT_IN_WITNESS_UTXO
			...
			txInsMeta[input].txOut.parse(val_arr + lenVarInt(v->length()),
						     v->length() - lenVarInt(v->length()));
		...
		case 2: { // PSBT_IN_PARTIAL_SIG
			...
			if (txInsMeta[input].signaturesLen == 0) {
				...
				txInsMeta[input].signatures =
					new PSBTPartialSignature[txInsMeta[input].signaturesLen];
			} else {
		...
		case 6: { // PSBT_IN_BIP32_DERIVATION
			...
			if (txInsMeta[input].derivationsLen == 0) {
				...
				txInsMeta[input].derivations =
					new PSBTDerivation[txInsMeta[input].derivationsLen];
			} else {
				PSBTDerivation *p = txInsMeta[input].derivations;
				txInsMeta[input].derivations =
					new PSBTDerivation[txInsMeta[input].derivationsLen + 1];
			...
			PSBTDerivation *der =
				&txInsMeta[input].derivations[txInsMeta[input].derivationsLen - 1];
			...
			der->derivationLen =
				(v->length() - lenVarInt(v->length()) - 4) / sizeof(uint32_t);
			der->derivation = (uint32_t *)calloc(der->derivationLen, sizeof(uint32_t));
			if (der->derivation == NULL) {
				der->derivationLen = 0;
				res = -1;
				break;
			}
			for (size_t i = 0; i < der->derivationLen; i++) {
				der->derivation[i] = littleEndianToInt(
					val_arr + lenVarInt(v->length()) + 4 * (i + 1), 4);
			}
			res = 1;
			break;
		}
		...
		case 2: { // PSBT_OUT_BIP32_DERIVATION
			...
			if (txOutsMeta[output].derivationsLen == 0) {
				...
				txOutsMeta[output].derivations =
					new PSBTDerivation[txOutsMeta[output].derivationsLen];
			} else {
				PSBTDerivation *p = txOutsMeta[output].derivations;
				txOutsMeta[output].derivations =
					new PSBTDerivation[txOutsMeta[output].derivationsLen + 1];
			...
			PSBTDerivation *der =
				&txOutsMeta[output]
					 .derivations[txOutsMeta[output].derivationsLen - 1];
			der->pubkey.parse(key_arr + 2, k->length() - 2);
			if (der->pubkey.getStatus() != PARSING_DONE) {
				res = -1;
				break;
	...
	while (s->available() && section < sections_number) {
		if (section > 0 && section < tx.inputsNumber + 1) {
			...
			for (size_t i = 0; i < txInsMeta[input].signaturesLen; i++) {
				...
				uint8_t key_len =
					1 + txInsMeta[input].signatures[i].pubkey.serialize(
						    key_arr + 2, 65);
				key_arr[0] = key_len;
				while (s->available() &&
				       bytes_written + offset - cur < (size_t)key_len + 1) {
				...
				uint8_t val_arr[100];
				uint8_t val_len =
					1 + txInsMeta[input].signatures[i].signature.serialize(
						    val_arr + 1, 98);
				val_arr[0] = val_len;
				val_arr[val_len] = SIGHASH_ALL;
				while (s->available() &&
				       bytes_written + offset - cur < (size_t)val_len + 1) {
...
PSBT::PSBT(PSBT const &other)
{
	...
	for (size_t i = 0; i < tx.inputsNumber; i++) {
		...
		for (size_t j = 0; j < txInsMeta[i].derivationsLen; j++) {
			...
			txInsMeta[i].derivations[j].derivation = (uint32_t *)calloc(
				txInsMeta[i].derivations[j].derivationLen, sizeof(uint32_t));
			if (txInsMeta[i].derivations[j].derivation == NULL) {
				txInsMeta[i].derivations[j].derivationLen = 0;
			} else {
				memcpy(txInsMeta[i].derivations[j].derivation,
				       other.txInsMeta[i].derivations[j].derivation,
				       txInsMeta[i].derivations[j].derivationLen *
					       sizeof(uint32_t));
			}
		}
	...
	for (size_t i = 0; i < tx.outputsNumber; i++) {
		...
		for (size_t j = 0; j < txOutsMeta[i].derivationsLen; j++) {
			...
			txOutsMeta[i].derivations[j].derivation = (uint32_t *)calloc(
				txOutsMeta[i].derivations[j].derivationLen, sizeof(uint32_t));
			if (txOutsMeta[i].derivations[j].derivation == NULL) {
				txOutsMeta[i].derivations[j].derivationLen = 0;
			} else {
				memcpy(txOutsMeta[i].derivations[j].derivation,
				       other.txOutsMeta[i].derivations[j].derivation,
				       txOutsMeta[i].derivations[j].derivationLen *
					       sizeof(uint32_t));
			}
		}
	}
...
uint8_t PSBT::sign(const HDPrivateKey root)
{
	...
	for (size_t i = 0; i < tx.inputsNumber; i++) {
		if (txInsMeta[i].derivationsLen > 0) {
			for (size_t j = 0; j < txInsMeta[i].derivationsLen; j++) {
				if (memcmp(fingerprint, txInsMeta[i].derivations[j].fingerprint,
					   4) == 0) {
					// caching account key here
					if (first_derivation == NULL) {
						first_derivation =
							txInsMeta[i].derivations[j].derivation;
						first_derivation_len = 0;
						for (size_t k = 0;
						     k < txInsMeta[i].derivations[j].derivationLen;
						     k++) {
							if (txInsMeta[i]
								    .derivations[j]
								    .derivation[k] >= 0x80000000) {
								first_derivation_len++;
							} else {
								break;
							}
						}
						account = root.derive(first_derivation,
								      first_derivation_len);
					}
					PrivateKey pk;
					// checking if cached key is ok
					if (memcmp(first_derivation,
						   txInsMeta[i].derivations[j].derivation,
						   first_derivation_len * sizeof(uint32_t)) == 0) {
						pk = account.derive(
							txInsMeta[i].derivations[j].derivation +
								first_derivation_len,
							txInsMeta[i].derivations[j].derivationLen -
					...
					if (txInsMeta[i].derivations[j].pubkey == pk.publicKey()) {
						// can sign - let's sign
						uint8_t h[32];
						if (txInsMeta[i].witnessScript.length() >
						    1) { // P2WSH / P2SH_P2WSH
							tx.sigHashSegwit(h, i,
									 txInsMeta[i].witnessScript,
									 txInsMeta[i].txOut.amount);
						} else {
							if (txInsMeta[i].redeemScript.length() >
							    1) {
								if (txInsMeta[i]
									    .redeemScript.type() ==
								    P2WPKH) { // P2SH_P2WPKH
						...
						uint8_t len =
							1 + pk.publicKey().serialize(arr + 2, 65);
						arr[0] = len;
						Script k;
						k.parse(arr, len + 1);
...
bool PSBT::isMine(uint8_t outputNumber, const HDPublicKey xpub) const
{
	...
	if (txOutsMeta[outputNumber].derivationsLen > 0) {
		for (unsigned int j = 0; j < txOutsMeta[outputNumber].derivationsLen; j++) {
			HDPublicKey pub =
				xpub.derive(txOutsMeta[outputNumber].derivations[j].derivation,
					    txOutsMeta[outputNumber].derivations[j].derivationLen);
			if (memcmp(pub.point, txOutsMeta[outputNumber].derivations[j].pubkey.point,
				   64) == 0) {
				mine = true;
			}
		}
...
bool PSBT::isMine(uint8_t outputNumber, const HDPrivateKey xprv) const
{
	...
	if (txOutsMeta[outputNumber].derivationsLen > 0) {
		for (unsigned int j = 0; j < txOutsMeta[outputNumber].derivationsLen; j++) {
			HDPublicKey pub =
				xprv.derive(txOutsMeta[outputNumber].derivations[j].derivation,
					    txOutsMeta[outputNumber].derivations[j].derivationLen)
					.xpub();
			if (memcmp(pub.point, txOutsMeta[outputNumber].derivations[j].pubkey.point,
				   64) == 0) {
...
PSBT &PSBT::operator=(PSBT const &other)
{
	...
	if (tx.inputsNumber > 0) {
		...
		for (size_t i = 0; i < tx.inputsNumber; i++) {
			txInsMeta[i] = other.txInsMeta[i];
			if (txInsMeta[i].derivationsLen > 0) {
				txInsMeta[i].derivations =
					new PSBTDerivation[txInsMeta[i].derivationsLen];
				for (size_t j = 0; j < txInsMeta[i].derivationsLen; j++) {
					txInsMeta[i].derivations[j] =
						other.txInsMeta[i].derivations[j];
					txInsMeta[i].derivations[j].derivation = (uint32_t *)calloc(
						txInsMeta[i].derivations[j].derivationLen,
						sizeof(uint32_t));
					if (txInsMeta[i].derivations[j].derivation == NULL) {
			...
			if (txInsMeta[i].signaturesLen > 0) {
				txInsMeta[i].signatures =
					new PSBTPartialSignature[txInsMeta[i].signaturesLen];
				for (size_t j = 0; j < txInsMeta[i].signaturesLen; j++) {
					txInsMeta[i].signatures[j] =
						other.txInsMeta[i].signatures[j];
				}
			}
		}
	}
	if (tx.outputsNumber > 0) {
		...
		for (size_t i = 0; i < tx.outputsNumber; i++) {
			txOutsMeta[i] = other.txOutsMeta[i];
			txOutsMeta[i].derivations =
				new PSBTDerivation[txOutsMeta[i].derivationsLen];
			for (size_t j = 0; j < txOutsMeta[i].derivationsLen; j++) {
				txOutsMeta[i].derivations[j] = other.txOutsMeta[i].derivations[j];
				txOutsMeta[i].derivations[j].derivation = (uint32_t *)calloc(
					txOutsMeta[i].derivations[j].derivationLen,
