...
TxIn::TxIn(const uint8_t prev_id[32], uint32_t prev_index, const Script script,
...
size_t TxIn::from_stream(ParseStream *s)
{
	...
	if (scriptSig.getStatus() == PARSING_DONE &&
	    bytes_read + bytes_parsed == 32 + 4 + scriptSig.length() + 4) {
		status = PARSING_DONE;
	}
	bytes_parsed += bytes_read;
	return bytes_read;
...
size_t TxOut::from_stream(ParseStream *s)
{
	...
	if (scriptPubkey.getStatus() == PARSING_DONE &&
	    bytes_read + bytes_parsed == 8 + scriptPubkey.length()) {
		status = PARSING_DONE;
	}
	bytes_parsed += bytes_read;
	return bytes_read;
...
size_t Tx::to_stream(SerializeStream *s, size_t offset) const
{
	...
	for (unsigned int i = 0; i < inputsNumber; i++) {
		...
		if (s->available() && bytes_written + offset < cur_offset + l) {
			bytes_written +=
				s->serialize(&txIns[i], bytes_written + offset - cur_offset);
		}
		cur_offset += l;
	}
	...
	for (unsigned int i = 0; i < outputsNumber; i++) {
		...
		if (s->available() && bytes_written + offset < cur_offset + l) {
			bytes_written +=
				s->serialize(&txOuts[i], bytes_written + offset - cur_offset);
		}
		cur_offset += l;
	}
	if (is_segwit) {
		for (unsigned int i = 0; i < inputsNumber; i++) {
			l = txIns[i].witness.length();
			if (s->available() && bytes_written + offset < cur_offset + l) {
				bytes_written += s->serialize(&txIns[i].witness,
							      bytes_written + offset - cur_offset);
			}
			cur_offset += l;
		}
	}
...
size_t Tx::from_stream(ParseStream *s)
{
	...
	if (s->available() && bytes_read + bytes_parsed == 4) {
		...
		if (c == 0x00) { // segwit!
			...
			inputsNumber =
				c; // FIXME: should be varint, but 255 inputs will kill the MCU...
			txIns = new TxIn[inputsNumber];
			for (unsigned int i = 0; i < inputsNumber;
			     i++) { // this will at least set all txins to PARSING_INCOMPLETE
	...
	if (s->available() && bytes_read + bytes_parsed == current_offset) {
		...
		bytes_read++;
		txOuts = new TxOut[outputsNumber];
		for (unsigned int i = 0; i < outputsNumber;
		     i++) { // this will at least set all txouts to PARSING_INCOMPLETE
	...
	if (segwit_flag > 0 && bytes_read + bytes_parsed == current_offset) {
		for (unsigned int i = 0; i < inputsNumber;
		     i++) { // this will at least set all txins witnesses to PARSING_INCOMPLETE
...
int Tx::sigHash(uint8_t h[32], uint8_t inputIndex, const Script scriptPubkey,
		SigHashType sighash) const
{
	Script empty;
	DoubleSha s;
	s.begin();
...
int Tx::sigHashSegwit(uint8_t h[32], uint8_t inputIndex, const Script scriptPubKey, uint64_t amount,
		      SigHashType sighash) const
{
	DoubleSha s;
	s.begin();
...
Signature Tx::signInput(uint8_t inputIndex, const PrivateKey pk, const Script redeemScript,
			SigHashType sighash)
{
	uint8_t h[32];
...
Signature Tx::signSegwitInput(uint8_t inputIndex, const PrivateKey pk, const Script redeemScript,
			      uint64_t amount, ScriptType type, SigHashType sighash)
{
	uint8_t h[32];
