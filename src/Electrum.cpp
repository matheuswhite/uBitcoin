...
size_t ElectrumTx::from_stream(ParseStream *s)
{
	...
	if (!is_segwit && s->available() && status == PARSING_INCOMPLETE &&
	    tx.getStatus() != PARSING_FAILED) {
		...
		if (tx.getStatus() == PARSING_DONE) {
			...
			for (unsigned i = 0; i < tx.inputsNumber; i++) {
				if (tx.txIns[i].scriptSig.length() !=
				    88) { // no idea how to parse other things
					status = PARSING_FAILED;
					bytes_parsed += bytes_read;
					return bytes_read;
				} else {
	...
	if (is_segwit && tx.getStatus() == PARSING_DONE) {
		...
		for (unsigned int i = 0; i < tx.inputsNumber; i++) {
			...
			while (s->available() && bytes_parsed + bytes_read < start + 8) {
				...
				txInsMeta[i].amount +=
					(((uint64_t)c)
					 << (8 * (bytes_read + bytes_parsed - start)));
				bytes_read++;
			}
			start += 8;
...
uint8_t ElectrumTx::sign(const HDPrivateKey account)
{
	...
	for (unsigned int i = 0; i < tx.inputsNumber; i++) {
		...
		if (pub == txInsMeta[i].hd) {
			PrivateKey pk = account.child(txInsMeta[i].derivation[0])
						.child(txInsMeta[i].derivation[1]);
			if (type == P2PKH || type == P2SH || type == UNKNOWN_TYPE ||
			    type == DIRECT_SCRIPT) {
				tx.signInput(i, pk);
			} else {
