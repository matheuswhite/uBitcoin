...
size_t fromBase64Length(const char *array, size_t arraySize, uint8_t flags)
{
	...
	if (arraySize % 4 != 0 && (flags & BASE64_NOPADDING) == 0) {
		return 0;
	}
...
{
	size_t len = fromBase64Length(b64.c_str(), strlen(b64.c_str()), flags) +
