...
class ECPoint : public Streamable
{
	...
	{
		return 33 + 32 * (1 - compressed);
	};
	virtual size_t stringLength() const
	{
		return 2 * ECPoint::length();
	};
	...
	{
		return (memcmp(point, other.point, 64) == 0);
	};
	bool operator!=(const ECPoint &other) const
	{
...
class ECScalar : public Streamable
{
	...
	ECScalar(const uint8_t *arr, size_t len)
	{
		init();
		parse(arr, len);
	};
	explicit ECScalar(const char *arr)
	{
		init();
		parse(arr, strlen(arr));
	};
	ECScalar(uint32_t i)
	{
		init();
		intToBigEndian(i, num, 32);
	};
	~ECScalar()
	{
		memzero(num, 32);
	...
	virtual void setSecret(const uint8_t secret_arr[32])
	{
		memcpy(num, secret_arr, 32);
	};
	/** \brief Sets the secret key */
	void getSecret(uint8_t buffer[32]) const
	{
		memcpy(buffer, num, 32);
	};
	...
	ECScalar operator+=(const uint32_t &i)
	{
		*this = *this + i;
		return *this;
	};
	...
	{
		return (memcmp(num, other.num, 32) == 0);
	};
	bool operator!=(const ECScalar &other) const
	{
		return !operator==(other);
...
inline ECScalar operator/(uint32_t i, ECScalar &scalar)
{
	return ECScalar(i) / scalar;
};
inline ECScalar operator/(ECScalar &scalar, uint32_t i)
{
	return scalar / ECScalar(i);
};
inline ECScalar operator*(uint32_t i, ECScalar &scalar)
{
	return ECScalar(i) * scalar;
...
inline ECPoint operator*(const ECPoint &p, const ECScalar &d)
{
	return d * p;
};
inline ECPoint operator/(const ECPoint &p, const ECScalar &d)
{
	return (ECScalar(1) / d) * p;
};
