...
class Readable
{
	...
	{
		return this->to_str(buf, len);
	};
	size_t fromString(const char *buf, size_t len)
	{
		return this->from_str(buf, len);
	};
	size_t fromString(const char *buf)
	{
		return this->from_str(buf, strlen(buf));
	};
#ifdef ARDUINO
...
class Streamable : public Readable
{
	...
	virtual void reset()
	{
		status = PARSING_DONE;
		bytes_parsed = 0;
	}; // used to reset parsing and mb object
	 ...
	 */
	parse_status getStatus()
	{
		return status;
	};
	/** \brief Sets parsing status. Should be used with care. */
	void setStatus(parse_status s)
	{
		status = s;
	};
...
#endif
	size_t serialize(uint8_t *arr, size_t len, size_t offset = 0,
			 encoding_format format = RAW) const;
	size_t parse(const char *arr, size_t len, encoding_format format = HEX_ENCODING)
	{
		return parse((const uint8_t *)arr, len, format);
	}
	size_t serialize(char *arr, size_t len, size_t offset = 0,
			 encoding_format format = HEX_ENCODING) const
	{
