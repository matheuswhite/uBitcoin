...
size_t mnemonicToEntropy(const char *mnemonic, size_t mnemonic_len, uint8_t *output,
			 size_t outputLen);
#if USE_ARDUINO_STRING
 ...
 *  PublicKey class.
 *
 *  Derived from ECPoint class, therefore you can add or substract them, multiply by ECScalar or
 * PrivateKey.
 *
 ...
 *  - `compressed = true` will use 33-byte representation (`03<x>` if y is odd, `02<x>` if y is
 * even)
 */
class PublicKey : public ECPoint
{
	...
	{
		reset();
		parse(secArr, 33 + ((uint8_t)(secArr[0] == 0x04)) * 32);
	};
	explicit PublicKey(const char *secHex)
	{
		reset();
		from_str(secHex, strlen(secHex));
	};
	PublicKey(const ECPoint p)
	{
		reset();
		memcpy(point, p.point, 64);
	 ...
	 */
	int nestedSegwitAddress(char *addr, size_t len,
				const Network *network = &DEFAULT_NETWORK) const;
	/**
	 *  \brief Alias for `legacyAddress`
	 */
	int address(char *address, size_t len, const Network *network = &DEFAULT_NETWORK) const
	{
		return legacyAddress(address, len, network);
	};
#if USE_ARDUINO_STRING
...
class PrivateKey : public ECScalar
{
	...
	{
		return fromWIF(buf, len);
	};
	virtual size_t from_stream(ParseStream *s);

      public:
	PrivateKey();
	PrivateKey(const uint8_t secret_arr[32], bool use_compressed = true,
	...
	void setSecret(const uint8_t secret_arr[32])
	{
	 ...
	 *  Optional parameter hash (true by default) defines if you want sha256(<x><y>) or just
	 * <x>. Having hash=true is recommended unless you have a very good reason not to use it.
	 */
 ...
 *  xprv and xpub methods return strings according to slip32, xprv/xpub for bip44, yprv/ypub for
 * bip49 and zprv/zpub for bip84
 */
class HDPrivateKey : public PrivateKey
{
	...
	uint8_t prefix[4]; // used for parsing only
      public:
	HDPrivateKey();
	HDPrivateKey(const uint8_t secret[32], const uint8_t chain_code[32], uint8_t key_depth = 0,
		     const uint8_t parent_fingerprint_arr[4] = NULL, uint32_t childnumber = 0,
		     const Network *network = &DEFAULT_NETWORK, ScriptType key_type = UNKNOWN_TYPE);
	HDPrivateKey(const char xprvArr[]);
	HDPrivateKey(const char *mnemonic, size_t mnemonicSize, const char *password,
		     size_t passwordSize, const Network *network = &DEFAULT_NETWORK,
		     void (*progress_callback)(float) = NULL);
#if USE_STD_STRING
	HDPrivateKey(std::string mnemonic, std::string password,
		     const Network *network = &DEFAULT_NETWORK,
		     void (*progress_callback)(float) = NULL);
#endif
#if USE_ARDUINO_STRING
	HDPrivateKey(String mnemonic, String password, const Network *network = &DEFAULT_NETWORK,
		     void (*progress_callback)(float) = NULL);
#endif
	...
	*/
	...
	int fromMnemonic(const char *mnemonic, size_t mnemonicSize, const char *password,
			 size_t passwordSize, const Network *network = &DEFAULT_NETWORK,
			 void (*progress_callback)(float) = NULL);
	int fromMnemonic(const char *mnemonic, const char *password,
			 const Network *network = &DEFAULT_NETWORK,
			 void (*progress_callback)(float) = NULL)
	{
		return fromMnemonic(mnemonic, strlen(mnemonic), password, strlen(password), network,
				    progress_callback);
	}
#if USE_STD_STRING
	int fromMnemonic(std::string mnemonic, std::string password,
			 const Network *network = &DEFAULT_NETWORK,
			 void (*progress_callback)(float) = NULL);
#endif
#if USE_ARDUINO_STRING
	int fromMnemonic(String mnemonic, String password,
			 const Network *network = &DEFAULT_NETWORK,
			 void (*progress_callback)(float) = NULL);
#endif
	...
	/** \brief derives a child according to derivation path. Use 0x80000000 + index for hardened
	 * index. */
	HDPrivateKey derive(uint32_t *index, size_t len) const;
	/** \brief derives a child according to derivation path. For example "m/84h/1h/0h/1/23/" for
	 * the 23rd change address for testnet with P2WPKH type (bip84). */
	HDPrivateKey derive(const char *path) const;
#if USE_ARDUINO_STRING
	HDPrivateKey derive(String path) const
	{
 ...
 *  xpub method return strings according to slip32, xpub for bip44, ypub for bip49 and zpub for
 * bip84
 */
class HDPublicKey : public PublicKey
{
	...
	uint8_t prefix[4]; // used for parsing only
      public:
	HDPublicKey();
	HDPublicKey(const uint8_t point[64], const uint8_t chain_code[32], uint8_t key_depth = 0,
		    const uint8_t parent_fingerprint_arr[4] = NULL, uint32_t childnumber = 0,
...
*/
	...
	/** \brief derives a child according to derivation path. For example "m/1/23/" for the 23rd
	 * change address. */
	HDPublicKey derive(const char *path) const;
#if USE_ARDUINO_STRING
	HDPublicKey derive(String path) const
	{
		return derive(path.c_str());
	};
#endif
...
class Signature : public Streamable
{
	...
	{
		uint8_t arr[32] = {0};
		return !((memcmp(r, arr, 32) == 0) && (memcmp(s, arr, 32) == 0));
	};
	...
	{
		return (memcmp(r, other.r, 32) == 0) && (memcmp(s, other.s, 32) == 0);
	};
	bool operator!=(const Signature &other) const
	{
...
class SchnorrSignature : public Streamable
{
	...
	bool operator==(const SchnorrSignature &other) const
	{
		return (memcmp(r, other.r, 32) == 0) && (memcmp(s, other.s, 32) == 0);
	};
	bool operator!=(const SchnorrSignature &other) const
...
class Script : public Streamable
{
...
#if USE_ARDUINO_STRING
	/** \brief creates a script from address */
	Script(const String address)
	{
		init();
		fromAddress(address.c_str());
	};
#endif
#if USE_STD_STRING
	/** \brief creates a script from address */
	Script(const std::string address)
	{
		init();
		fromAddress(address.c_str());
	};
	...
	// Bool conversion. Allows to use if(script) construction. Returns false if script is empty,
	// true otherwise
	explicit operator bool() const
	{
		return (scriptLen > 0);
	...
	{
		return (scriptLen == other.scriptLen) &&
		       (memcmp(scriptArray, other.scriptArray, scriptLen) == 0);
	};
	bool operator!=(const Script &other) const
	{
		return !operator==(other);
...
class Witness : public Streamable
{
	...
	{
		return (witnessLen == other.witnessLen) &&
		       (memcmp(witnessArray, other.witnessArray, witnessLen) == 0) &&
...
class TxIn : public Streamable
{
	...
	void init();
      ...
      public:
	TxIn(void);
	TxIn(const uint8_t prev_id[32], uint32_t prev_index, const Script script,
	     uint32_t sequence_number = 0xffffffff);
	TxIn(const uint8_t prev_id[32], uint32_t prev_index, uint32_t sequence_number = 0xffffffff);
	explicit TxIn(const char *prev_id, uint32_t prev_index, const Script script,
		      uint32_t sequence_number = 0xffffffff);
	explicit TxIn(const char *prev_id, uint32_t prev_index,
		      uint32_t sequence_number = 0xffffffff);
	virtual size_t length() const;
	uint8_t hash[32];
...
class TxOut : public Streamable
{
	...
	TxOut(uint64_t send_amount, const Script outputScript)
	{
		amount = send_amount;
		scriptPubkey = outputScript;
	};
	TxOut(const Script outputScript, uint64_t send_amount)
	{
		amount = send_amount;
		scriptPubkey = outputScript;
	};
	TxOut(uint64_t send_amount, const char *address)
	{
		amount = send_amount;
		scriptPubkey = Script(address);
	};
	TxOut(const char *address, uint64_t send_amount)
	{
		amount = send_amount;
		scriptPubkey = Script(address);
	};
	...
	size_t address(char *addr, size_t len, const Network *network = &DEFAULT_NETWORK) const
	{
		return scriptPubkey.address(addr, len, network);
	};
#if USE_ARDUINO_STRING
	String address(const Network *network = &DEFAULT_NETWORK) const
	{
		return scriptPubkey.address(network);
	};
#endif
#if USE_STD_STRING
	std::string address(const Network *network = &DEFAULT_NETWORK) const
 ...
 *         Can be segwit or not. For legacy tx serializes as
 * `<ver><inputsNumber><inputs><outputsNumber><outputs><locktime>`<br> For segwit tx serializes as
 * `<ver><00><01><inputsNumber><inputs><outputsNumber><outputs><witnesses><locktime>`
 */
class Tx : public Streamable
{
	...
	int sigHash(uint8_t h[32], uint8_t inputIndex, const Script scriptPubkey,
	...
	int sigHashSegwit(uint8_t h[32], uint8_t inputIndex, const Script scriptPubKey,
			  uint64_t amount, SigHashType sighash = SIGHASH_ALL) const;
	 ...
	 */
	Signature signInput(uint8_t inputIndex, const PrivateKey pk, const Script redeemScript,
	 ...
	 *         Don't forget to construct txIns[i].witness correctly if you are using P2WSH or
	 * P2SH-P2WSH. For P2PKH and P2SH use signInput method.
	 */
	Signature signSegwitInput(uint8_t inputIndex, const PrivateKey pk,
				  const Script redeemScript, uint64_t amount,
				  ScriptType type = P2WSH, SigHashType sighash = SIGHASH_ALL);
	/** \brief signs segwit input and returns a signature. Uses native segwit (P2WPKH) by
	 * default, you can also specify the type to be P2SH-P2WPKH to sign nested segwit
	 * transaction.
	 */
	Signature signSegwitInput(uint8_t inputIndex, const PrivateKey pk, uint64_t amount,
				  ScriptType type = P2WPKH)
	{
		return signSegwitInput(inputIndex, pk, Script(pk.publicKey(), P2WPKH), amount,
				       type); // FIXME: are you sure?
	};
